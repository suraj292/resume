          ...
          <div class="max-w-xl mx-auto py-6 sm:py-10 px-4 sm:px-6 pb-24 lg:pb-10">
            ...
            <div v-show="activeTab === 'colors'" class="tab-content">
              ...
              <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <button v-for="color in colorPalettes" :key="color.id" @click="selectColor(color)" :class="['color-card group text-left bg-white border-2 p-4 rounded-2xl transition-all', selectedColor === color.id ? 'color-card-active border-indigo-500' : 'border-slate-100 hover:border-' + color.id + '-100']">
                  <div class="flex items-center gap-3 mb-3">
                    <div :class="`w-8 h-8 rounded-full bg-${color.id}-600 shadow-lg shadow-${color.id}-100`"></div>
                    <div :class="`w-8 h-8 rounded-full bg-${color.id}-50 border border-${color.id}-100`"></div>
                  </div>
